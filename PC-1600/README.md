...
| [`PC-1600-Memory-Architecture.md`](PC-1600-Memory-Architecture.md) | **complete** | Narrative / comparison layer: Z-80 8-bank view, internal-RAM layout (C5H reserve, work area, overhead), `NEW "S0:/S1:/S2:"`, module placement in the 8000–BFFF window + BASIC-area maps with/without modules and `NEW` (§4b, TRM §3.12–§3.13), the LH5803 compatibility memory map, address-by-address comparison to PC-1500/1500A, MODE0/MODE1, `ERROR 110` analysis (§8). TRM §2.1/§2.2/§3.12/§3.13/§5.15 + German *Bedienungsanleitung* + Operation Manual App. F/H. |
...
| [`PC-1600-Work-Area-Map.md`](PC-1600-Work-Area-Map.md) | **first pass** | The F000H–FFFFH BASIC/IOCS work area: 5-block structure (Block A sub-blocks corrected against the English TRM §6.1 figure), downward extension for peripheral ROM work areas + the PTR1–PTRG pointer table, the §6.3 named-variable map (LCD/keyboard/interpreter/error-capture/variable-storage/plotter state), and (§4) the `ADTBL`/`SxMTb` BASIC-program bank-distribution bytes + byte encoding (TRM §3.12.2). TRM Chapter 6 (English PDF pp.203–213) + §3.12.2. Complements `PC-1600-Memory-Bank-Switching.md` Part 6 (slot/boot addresses). |
| [`PC-1600-BASIC-Program-Placement.md`](PC-1600-BASIC-Program-Placement.md) | **complete** | Implementation spec for an emulator: how `LOAD` scatters one tokenised BASIC program across S0's bank list via `ADTBL`, reconstructed as an injection algorithm — build the segment list from `S0MTb`/`ADTBL`, top-justified module window bases, `NEW`/reserve holes, a physical `(bank,addr)` write cursor, pointer fix-ups, and direct-copy vs. simulated-store (`SLOT1MAP`/`SLOT2MAP` need no accounting). |
