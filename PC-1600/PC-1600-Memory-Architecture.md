...
**(d) Slot 2.** Slot 2's RAM is in **Bank 2/3**, not the default bank, so *something* has to repoint page-2 at Bank 2 and leave it there — and boot-time module detection does exactly that, **for every RAM module**:

- **Flat module (CE-1600M; a CE-161).** Folded into the work area **automatically at reset**, exactly as in Slot 1. `MEM` = 77 370 straight after reset with a CE-1600M in each slot (emulator).
- **Vertical-banked module (CE-1601M, CE-1650M, *superRAM*, 128 KB+).** Also merged **automatically at reset** — its **vertical bank 0** (32 KB) becomes expansion memory; the other vertical banks are RAM-disk space only. `MEM` = 77 370 straight after reset with a CE-1600M in Slot 1 and a CE-1601M in Slot 2 (emulator), and the real-hardware 128 KB Slot 2 + CE-155 test gave 52 794, i.e. a full 32 KB from Slot 2 (`PC-1600-Memory-Bank-Switching.md` Part 2).

In both cases `INIT "S2:","M"` is a no-op on a freshly merged module; it only matters to *re-type* a module that is currently a program module (`"P"`) or a RAM disk (`"F"`) back to expansion memory. After the merge the Z-80 map `8000–BFFF(C) → C000–FFFF(internal)` is flat — same picture as (c).

The structural Slot-1/Slot-2 asymmetry (`PC-1600-Memory-Bank-Switching.md` Part 2) is only about the *ceiling* — Slot 2 can contribute at most one 32 KB vertical bank to `MEM` regardless of module size — not about whether a command is required: every RAM module auto-merges.
