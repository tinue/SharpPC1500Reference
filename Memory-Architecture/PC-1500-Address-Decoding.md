...
The mirroring gives a concrete answer, and the ROM situation makes it a hard constraint rather than a mere possibility: the PC-1500 and PC-1500A run the **identical** system ROM — not a ported or adapted variant. (The PC-1500 briefly shipped with earlier ROM revisions A01 and A03 before A04 became standard; the PC-1500A shipped with A04 from the start, and A04 is confirmed to run unmodified on both models. Whether A03 would run on a PC-1500A is untested.) Since A04 is the exact same set of bytes executing on both machines, any place that ROM touches address &7C00 behaves identically in both cases from the CPU's point of view — except that on the PC-1500, &7C00 is an alias for &7800, while on the PC-1500A it's an independent byte. Withholding &7C00 from the newly-exposed machine-language area guarantees that whatever the shared ROM does when it touches that specific address produces consistent results on both machines, without requiring any model-specific branch in the firmware. Everything from &7C01 onward was never aliased to anything on the PC-1500, so the identical ROM was never depending on it, and it's safe to expose as new, independent, user-accessible space.

Sharp didn't design two independent RAM expansions: they replaced small, oddly-sized parts (one HM6116 + two TC5514) with four of one cheap, standard, off-the-shelf chip, and exposed what that gave them beyond &7C00 — the one byte the shared ROM might still touch expecting the old alias to &7800. The pin-16/17/18 rewiring in §3 was the necessary side effect on the module-facing side of that swap; the machine-language area was the side effect on the system-RAM side, with &7C00 itself withheld to keep the identical ROM's behavior consistent across both models.
...
**"&7C01 (rather than &7C00) is a reserve-area convention, matching the same off-by-one pattern seen elsewhere in this document (e.g. `NEW &C5`)."** This looked attractive because the document already establishes a real, repeated pattern of "reserve up to and including a marker byte, then usable space starts one byte later," and &7C01 fits that shape numerically. It's wrong, or at least unmotivated: `NEW &C5`'s 197-byte reserve is a BASIC-interpreter convention describing where a *program* may safely start within a bank — it has no connection to why one specific *hardware* byte at the edge of the S7 block would need protecting. The better-supported explanation (§4.2) is that &7C00 specifically aliases &7800 on the plain PC-1500 (the TC5514 pair's missing 11th address line mirrors the block's two halves), and since the PC-1500 and PC-1500A run the identical system ROM (A04), reserving exactly that one byte keeps the shared ROM's behavior consistent across both machines, while the rest of the newly-independent region — never aliased to anything on the PC-1500, so never depended upon by the shared ROM — is safe to expose. The lesson: a numeric pattern matching a real convention elsewhere in the same system is suggestive, not sufficient, when a more specific hardware mechanism (here, address-line aliasing) directly explains the same number.
